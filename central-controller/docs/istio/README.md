...
These steps need to be followed in the Kubernetes Cluster where SDEWAN-CNF, SDEWAN-CRD-CONTROLLER and SDEWAN-OVERLAY-CONTROLLER is installed.

### Pre-Installation

Note: The `url` in the following configurations are recommended to be set as node port ip of the related service. For example, `<keycloak-url>` can be found from the command `kubectl get service -A`.
...
- Configure and setup keycloak in k8s cluster
  ...
  # Create secret for keycloak (key and cert are created in previous step)
