...
function get_uci_section(configuration, validator, name)
        ...
        if(type(v) == "table") then
            ...
            if value ~= nil then
                ...
                if v["item_validator"] ~= nil and type(v["item_validator"]) == "table" then
                    ...
                    local index = 1
                    for j=1, #value do
                        local item_obj = get_uci_section(configuration, v["item_validator"], value[j])
                    ...
                    if v["validator"] ~= nil and type(v["validator"]) == "table" then
                        target[name] = get_uci_section(configuration, v["validator"], value)
                    else                  
                        target[name] = value
                    end
                end
...
function get_object(module_table, processors, object_type, name)
    if processors ~= nil and processors[object_type] ~= nil 
       ...
       and module_table[processors[object_type]["get"]] ~= nil then
        ...
        return get_uci_section(processors["configuration"], processors[object_type].validator, name)
    end
end
                    ...
                    if type(obj_value) == "table" then
                        if v["item_validator"] ~= nil and type(v["item_validator"]) == "table" then
                            ...
                            for j=1, #obj_value do
                                sub_objs[j] = obj_value[j].name
                                local res, msg = create_uci_section(configuration, v["item_validator"], name, obj_value[j])
                                if res == false then
                                    return res, msg
                                end
                            end
                            uci:set_list(configuration, obj_section, name, sub_objs)
                        else
                            uci:set_list(configuration, obj_section, name, obj_value)
                        end
                    else
                        if v["validator"] ~= nil and type(v["validator"]) == "table" then
                            local res, msg = create_uci_section(configuration, v["validator"], name, obj_value)
                            if res == false then
                                return res, msg
                            end
                            uci:set(configuration, obj_section, name, obj_value.name)
                        else
                            if obj_value ~= "" then
                                uci:set(configuration, obj_section, name, obj_value)
                            end
                        end
                    end
                end
