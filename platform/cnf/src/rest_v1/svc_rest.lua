...
function create_service()
    ...
    local file = io.open("/etc/svc.info", "a+")
...
function delete_service()
    ...
    local info_file = io.open("/etc/svc.info", "w")
    local up_file = io.open("/etc/svc.up", "r")
...
function get_service()
    ...
    local file = io.open("/etc/svc.info", "r")
    if #uri_list == 6 then
...
-- Sync and validate
function sync_info()
    local in_file = io.open("/etc/svc.info", "r")
    local out_file = io.open("/etc/svc.up", "w")
