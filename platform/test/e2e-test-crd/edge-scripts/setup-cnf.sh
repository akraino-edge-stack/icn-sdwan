...
echo "--------------------- Clean default settings --------------------------------"
cnf_pod=$(kubectl get po | grep $sdewan_cnf_name | awk '{print $1}')
...
EOF
kubectl exec -it $cnf_pod -- opkg install tar
kubectl cp include $cnf_pod:/etc/config/firewall
