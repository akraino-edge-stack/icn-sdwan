...
func setStatus(instance runtime.Object, isSync bool) {
	value := reflect.ValueOf(instance)
	field_rv := reflect.Indirect(value).FieldByName("Generation")
	rv := field_rv.Interface().(int64)
	field_status := reflect.Indirect(value).FieldByName("Status")
	status := field_status.Interface().(batchv1alpha1.SdewanStatus)
	if isSync {
		status.AppliedGeneration = rv
...
func setMessage(instance runtime.Object, isSync bool, msg string) {
	value := reflect.ValueOf(instance)
	field_rv := reflect.Indirect(value).FieldByName("Generation")
	rv := field_rv.Interface().(int64)
	field_status := reflect.Indirect(value).FieldByName("Status")
...
func ProcessReconcile(r client.Client, logger logr.Logger, req ctrl.Request, handler basehandler.ISdewanHandler) (ctrl.Result, error) {
	...
	if err != nil {
		...
		setMessage(instance, false, err.Error())
